...
// Colors generator with chroma laibrary
...
function randomColors () {
    colorDives.forEach((div,index) => {
       ...
       colorizeSlisers(color, hue, brightness, saturation);
    });
}
...
function colorizeSlisers(color, hue, brightness, saturation) {
    //Scale saturation
...
function hslControls(e) {
    ...
    let sliders = e.target.parentElemenet.querySelectorAll('input[type="range"]');
    const hue = sliders[0];
    const brightness = sliders[1];
    const saturation = sliders[2];

    const bgColor = colorDives[index].querySelector("h2").innerText;
    ...
    colorDives[index].style.backgroundColor = color;
}
